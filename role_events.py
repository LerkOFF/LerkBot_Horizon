...
async def on_member_update(before, after):
    ...
    if added_tracked_roles:
        ...
        try:
            ...
            logger.warning(f"Не удалось отправить личное сообщение пользователю {after.name}. Личные сообщения отключены.")
        ...
        if boosty_role:
            try:
                ...
                logger.info(f"BOOSTY_ROLE_ID ({boosty_role.name}) добавлена пользователю {after.name}")
                log_user_action(f"BOOSTY_ROLE_ID ({boosty_role.name}) добавлена пользователю {after.name}", after)
            except Exception as e:
                logger.error(f"Не удалось добавить роль BOOSTY_ROLE_ID: {e}")
                log_user_action(f"Ошибка при добавлении BOOSTY_ROLE_ID пользователю {after.name}: {e}", after)
        else:
            logger.warning(f"Роль BOOSTY_ROLE_ID={BOOSTY_ROLE_ID} не найдена в гильдии {after.guild.name}")
    ...
    if removed_tracked_roles:
        ...
        try:
            await after.send(
                f"Видимо Ваша подписка на бусти **https://boosty.to/ss14.starhorizon** закончилась, так как вы потеряли роль: {removed_tracked_roles[0].name}.")
        except discord.Forbidden:
        ...
        async with _sponsors_file_lock:
            try:
                ...
                filtered_lines = [line for line in lines if not line.startswith(f"{after.name},")]
                ...
                try:
                    ...
                    logger.info(f"Пользователь {after.name} удалён из файла спонсоров")
                except Exception as e:
        ...
        if boosty_role:
            try:
                ...
                logger.info(f"BOOSTY_ROLE_ID ({boosty_role.name}) удалена у пользователя {after.name}")
                log_user_action(f"BOOSTY_ROLE_ID ({boosty_role.name}) удалена у пользователя {after.name}", after)
            except Exception as e:
                logger.error(f"Не удалось удалить роль BOOSTY_ROLE_ID: {e}")
