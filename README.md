...
Если URL или токен пустые, бот работает как раньше: только Boosty и `/my_ckey`. Sync **не** удаляет людей, которых нет в ответе сайта. Снимает роль только по списку `revoked`. Boosty-тексты не подменяются.
