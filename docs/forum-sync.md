...
- Discord → сайт: новые треды, сообщения, правки, удаления, архив/лок. Сайт идемпотентен по `discord_thread_id` / `discord_message_id`. Картинки сайт качает сам с Discord CDN. Уведомления на сайте не шлются.
