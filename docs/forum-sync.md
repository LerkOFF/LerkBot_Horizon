...
- Discord → сайт: новые треды, сообщения, правки, удаления, архив/лок. Сайт идемпотентен по `discord_thread_id` / `discord_message_id`. Картинки сайт качает сам с Discord CDN. Кастомные эмодзи гильдии бот заливает `PUT /internal/forum-emojis` сразу при старте и дублирует полем `emojis` в каждом PUT темы (`name`, `id`, `animated`); сайт рисует `:имя:` картинкой с CDN, уже залитые посты без повторного импорта. Гифки Tenor/Giphy в тексте сообщения сайт показывает как плеер; если в Discord только embed без ссылки, бот кладёт URL картинки в `attachments`. Уведомления на сайте не шлются.
